...
describe Event::Render::AddAdminSelect do
  before_all do
    @admin = create :user
    @user1 = create :user
    @user2 = create :user
  ...
  let(:text) { 'Select a User to promote' }
  let(:inline_keyboard) do
    [
      [{ text: user1.tag, callback_data: "add_event_admin:#{user1.id}" }],
      [{ text: user2.tag, callback_data: "add_event_admin:#{user2.id}" }]
    ]
  end
