...
describe Event::Response::Close::Confirmation::Success, response: true do
  include_examples 'response fixture event user'

  describe 'render Shared::Render::Confirmation' do
    include_examples '#render' do
      let(:params) { { positive_callback: { callback_data: "close_event:#{event.id}" }, current_user: current_user } }
