  ...
  describe '.call' do
    ...
    describe 'valid params' do
      describe 'create new Debt without coefficient' do
        let(:params) { { borrower: borrower, creditor: creditor, debt: Money.new(30_000, 'RUB') } }

        it { should be_success }
        it { expect { operation }.to change(Debt, :count).by(1) }
        it { expect(operation[:model].debt.format).to eq '300.00 ₽' }
      end

      describe 'create new Debt with coefficient' do
        let(:params) do
          { borrower: borrower, coefficient: 0.5, creditor: creditor, debt: Money.new(30_000, 'RUB') }
        end

        it { should be_success }
        it { expect { operation }.to change(Debt, :count).by(1) }
        it { expect(operation[:model].debt.format).to eq '150.00 ₽' }
      end

      describe 'accelerate debt' do
        let(:params) { { creditor: creditor, borrower: borrower, debt: Money.new(30_000, 'RUB') } }
        before do
        ...
        it { expect(operation[:model].debt.format).to eq '600.00 ₽' }
      end
    end
    ...
    describe 'invalid params' do
      describe 'empty debt value' do
        ...
        it { expect(operation_errors(operation)).to include 'Debt invalid, must be greater then 0' }
        it { expect { operation }.not_to change(Debt, :count) }
      end
    end
  end
